...
sub dns{
    ...
    # Bind socket DULU sebagai root (timeout=0 = non-blocking, hanya setup socket)
    $server->loop_once(0);

    # drop root privileges SETELAH socket terbind
    ...
    # Loop utama - proses tidak exit, handle request terus
    while (1) {
        $server->loop_once(10);
    }
